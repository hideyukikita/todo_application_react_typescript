...
## 2.1 データベース物理設計 (PostgreSQL / node-postgres)
※実装の安定性を考慮し、ORM(Prisma)を通さないpg(node-postgres)による直接クエリ方式を採用。
### User（ユーザー）
| カラム名 | 型 | 制約 | 説明 |
|---------| --- | ----- | ------ |
| id | UUID | PK | 主キー |
...
| deadline | DateTime | Not Null | 締切日(時刻含む、保存はUTC,取得時にJST変換) |
...
- **通信制御**
   - `Connect-SWL.ps1`によるWSL2からWindowsへのポートマッピング自動化。
   - フロントエンド`.env`にて物理IPアドレスを`VITE_API_URL`として定義。
